...
INPUT:
    ...
    The choice of Kmax: The simplest way is 
    maxtime=length(serie); 
    Kmax=floor(maxtime/2);
    The second way is to compute the estimates for increasing values of Kmax 
	and use the value where the estimates reach a plateau.
	The third way is that you decide it by yourself.
OUTPUT:
